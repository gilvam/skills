...
description: Generates Angular code and provides architectural guidance across the framework. Trigger when creating, editing, or refactoring an Angular project, component, directive, pipe, or service in a new or existing app, running ng generate/ng new/ng build, or for best practices on reactivity (signals, linkedSignal, resource, effect), forms (signal/reactive/template-driven), dependency injection, routing and guards, SSR/hydration, accessibility (Angular Aria), animations, styling (component styles, Tailwind CSS), unit or e2e testing, or CLI tooling and migrations. After generating or changing code, run the closing pass — code-standards-en, code-smell, and typescript-sonarqube — before reporting done. Do not use for HTTP service module scaffolding (use angular-http), application folder layout (use angular-folder-structure), or component/template/reactivity pattern reviews (use angular-patterns).
license: MIT
metadata:
