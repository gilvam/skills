...
description: Applies this project's TypeScript house standards — kebab-case filenames matching the symbol, one declaration per file with a type suffix (.model.ts/.interface.ts/.enum.ts), classes for domain models instantiated with new, default-initialized fields over optional (?) members, IName interfaces for implements contracts, enums for closed value sets, no any (unknown + guard at boundaries), array methods over for/while loops, .at() over bracket indexing, braces on every if, and a unit test per unit. Pulls current TypeScript guidance via the context7 ctx7 CLI. Use when creating, editing, refactoring, or reviewing a .ts file — a class/interface/enum, naming a file or type, setting a default value for a class field, or replacing a loop with array methods — in a new or existing project. After writing or changing code, close with code-standards-en, code-smell, and typescript-sonarqube. Defers identifier casing to code-standards-en, Angular folder layout to angular-folder-structure, and HTTP DTO modules to angular-http.
