---
name: angular-http
description: Create standardized Angular HTTP integration modules in a services/http folder placed closest to its consumer — with typed HttpClient services, @Dto() DTOs with explicit create()/createArray() mapping (keyCamelCase maps non-camelCase API keys like snake_case to camelCase; defaultValues backfills missing/empty fields of a partial response from a default JSON), a mock service, and unit tests. Pulls current Angular patterns via the context7 ctx7 CLI and guards the @Dto decorator dependency. Use when generating, editing, refactoring, or standardizing Angular REST/HTTP calls, creating request/response DTOs, mapping nested or non-camelCase API payloads, backfilling a partial API response from a default JSON, or adding HTTP mocks and service specs — in a new or existing project. Close with code-standards-en, code-smell, and typescript-sonarqube. Do not use for general component/template patterns (use angular-patterns) or application folder structure (use angular-folder-structure).
