...
description: Apply current Angular best practices when writing or reviewing component, template, reactivity, and form code — standalone components split into separate .ts/.html/.css files, signals for state with RxJS for streams (async/await only for one-shot promises), OnPush/zoneless change detection, native control flow (@if/@for/@switch), inject(), host bindings, class/style bindings over NgClass/NgStyle, and typed reactive forms. Pulls current Angular patterns via the Angular CLI MCP (get_best_practices) with the context7 ctx7 CLI as fallback. Use when authoring or reviewing Angular components, directives, or pipes; writing or reviewing a template; migrating legacy patterns (*ngIf/*ngFor, NgModule, constructor injection, NgClass/NgStyle) to current idioms; deciding how to model state (signals vs RxJS vs async/await); or building a reactive form. Do not use for folder layout (use angular-folder-structure) or HTTP service modules (use angular-http).
