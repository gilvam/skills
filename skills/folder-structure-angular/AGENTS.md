...
| `references/project-structure.md` | The canonical `core`/`shared`/`features` layout and routing. |
...
3. **Apply the layout** — place `core/`, `shared/`, `features/<feature>/` under `src/app`; wire routing with
...
- **Feature isolation.** Everything a feature needs lives inside its folder until a *second* consumer
  appears — only then promote it to `shared/` (presentational) or `core/` (singleton).
- **`core` vs `shared`.** `core/` = app-wide singletons provided once (guards, interceptors, app config,
  cross-cutting services). `shared/` = reusable, stateless presentational components/pipes/directives.
...
- No feature-specific code in `core/`/`shared/`; no singleton state in `shared/`.
